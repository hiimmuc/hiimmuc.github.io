...
def extract_author_info(config):
    ...
    if "author" in config:
        ...
        if author.get("googlescholar"):
            profiles.append(
                {"network": "Google Scholar", "username": "", "url": author.get("googlescholar")}
            )
        ...
        if author.get("researchgate"):
            profiles.append(
                {"network": "ResearchGate", "username": "", "url": author.get("researchgate")}
            )
...
def parse_skills(skills_text):
    ...
    for category, skills in categories:
        ...
        skills_entries.append({"name": category.strip(), "level": "", "keywords": skill_list})
